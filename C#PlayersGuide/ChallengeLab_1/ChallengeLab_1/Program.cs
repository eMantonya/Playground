    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            LoanApproval(monthlyPayment, monthlyIncome);
        ...
        static double CalculatePayment(double totalLoan, double escrow, double interest, double term, double closingCost)
        {
            ...
            double baseAmount = (monthlyIntrest * totalLoan) / (1 - Math.Pow(1 + monthlyIntrest, -numberOfPayments));
            return baseAmount + escrow + (closingCost / 12);

        }

        static void LoanApproval(double payment, double income)
        {
            if (payment > (income * .25))
            {
                Console.WriteLine("Deny that fool.");
            }
            else
            {
                Console.WriteLine("Approved! Take their money.");
            }
        }
    }
